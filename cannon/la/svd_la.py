...
def get_la_weights(vl, sl, u_l, len_samples, n_features):
    ...
    Q = sl.dot(vl.transpose()).dot(R.todense().transpose()).dot(vl).dot(la.inv(sl))
    B = Q.transpose()
...
class LinearAutoencoder:
    ...
    def fit(self, data):
        ...
        V, s, Uh = la.svd(Xhi, full_matrices=False)
        V = V[:, :p]
        s = s[:p]
        Uh = Uh[:p, :]
        ...
        Q = np.diag(s) @ V.T @ R.T @ V @ np.diag(inv_s)
    ...
    def decode(self, y, n):
        ...
        reco = []
        for i in range(n):
            reco.append(y @ self.A + self.mean)
            y = y @ self.B
        reco = np.stack(reco)
...
class IncrementalLinearAutoencoder(LinearAutoencoder):
    ...
    def partial_fit(self, data):
        ...
        Args:
            data: numpy array of shape (batch, time, features)
        """
        data = np.copy(data)
